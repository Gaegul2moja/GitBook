...
* 스레드(thread)): 애플리케이션의 작업 단위
    * 어플리케이션 안의 모든 컴포넌트들은 동일한 프로세스의 동일한 스레드로 실행된다. 이 스레드르 메인 스레드라고 부른다.
    * 메인 스레드는 사용자 인터페이스 위젯에게 이벤트를 전달하거나 화면을 그리는 작업을 담당하며, UI thread라고도 불린다.
    * UI스레드는 블록시키면 안되며, 외부에서 툴킷을 조작하면 안된다.
